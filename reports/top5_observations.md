...
The `exit` role appears in every processed trajectory (500/500 for each model). This is expected for mini-SWE-agent-v2 — the agent always emits a final exit payload carrying the patch or termination reason. I kept it separate from the required four categories because it is a termination payload rather than an assistant/tool turn. GPT-5-2-Codex also uses Responses API objects in the message array, so the parser normalizes response objects as assistant turns and function-call outputs as tool turns.
